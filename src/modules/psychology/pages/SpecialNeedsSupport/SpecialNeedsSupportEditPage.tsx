...
import { classroomService } from '../../../academic/service/Classroom.service';
...
export function SpecialNeedsSupportEditPage() {
  ...
  // Cargar instituciones cuando se carga el componente
  const loadReferenceData = async () => {
    try {
      // Load institutions
      const institutionsData = await institutionService.getAllInstitutions();
      setInstitutions(institutionsData.map((inst: any) => ({
        id: inst.institutionId,
        name: inst.institutionInformation.institutionName
      })));
  ...
  // Cargar aulas cuando se selecciona una institución
  useEffect(() => {
    const loadClassrooms = async () => {
      if (formData?.institutionId) {
        try {
          const institutionData = await institutionService.getInstitutionById(formData.institutionId);
  ...
  useEffect(() => {
    const loadStudents = async () => {
      if (formData?.classroomId) {
        try {
          const studentsData = await studentsService.getByClassroom(formData.classroomId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let errorMessage = 'No se pudo actualizar el soporte.';
