...
export function SpecialNeedsSupportDetailPage() {
  ...
  const loadSupport = async (supportId: string) => {
    try {
      ...
      let studentName = `Estudiante ${data.studentId.substring(0, 8)}`;
      let institutionName = `Institución ${data.institutionId.substring(0, 8)}`;
      let classroomName = `Aula ${data.classroomId.substring(0, 8)}`;
      
      try {
        // Load student name
        const student = await studentsService.getById(data.studentId);
        studentName = `${student.personalInfo.names} ${student.personalInfo.lastNames}`;
        
        // Load institution name
        const institution = await institutionService.getInstitutionById(data.institutionId);
        institutionName = institution.institutionInformation.institutionName;
        
        // Load classroom name
        if (institution.classrooms) {
          const classroom = institution.classrooms.find(c => c.classroomId === data.classroomId);
          if (classroom) {
      ...
      setSupport({
        ...
        classroomName,
        institutionName,
        diagnosedByName: data.diagnosedBy,
      });
    } catch (error) {
