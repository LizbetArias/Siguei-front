...
const SpecialNeedsSupportList: React.FC<SpecialNeedsSupportListProps> = ({ includeInactive = false }) => {
  ...
  const enrichWithNames = async (supportsList: SpecialNeedsSupport[]): Promise<EnrichedSupport[]> => {
    // Create a cache for institution, classroom, and student data to avoid duplicate requests
    const institutionCache: Record<string, { name: string; type: string; classrooms?: any[] }> = {};
    const classroomCache: Record<string, { name: string; gradeLevel?: string }> = {};
    const studentCache: Record<string, { names: string; lastNames: string }> = {};

    const enrichedSupports = await Promise.all(
      supportsList.map(async (support) => {
        try {
          // Get institution info if not cached
          if (!institutionCache[support.institutionId]) {
            try {
              const institution = await institutionService.getInstitutionById(support.institutionId);
              institutionCache[support.institutionId] = {
                name: institution.institutionInformation.institutionName,
                type: institution.institutionInformation.institutionType,
                classrooms: institution.classrooms || []
              };
            } catch (error) {
              console.warn('Error loading institution:', error);
              institutionCache[support.institutionId] = {
                name: `Institución ${support.institutionId.substring(0, 8)}`,
                type: 'Desconocido',
                classrooms: []
              };
            }
          ...
          if (!studentCache[support.studentId]) {
            try {
              const student = await studentsService.getById(support.studentId);
              studentCache[support.studentId] = {
                names: student.personalInfo.names,
                lastNames: student.personalInfo.lastNames,
              };
            } catch (error) {
              console.warn('Error loading student:', error);
              studentCache[support.studentId] = {
                names: 'Estudiante',
                lastNames: support.studentId.substring(0, 8),
              };
            }
          }

          // Get classroom name from institution data
          let classroomName = `Aula ${support.classroomId.substring(0, 8)}`;
          const institutionData = institutionCache[support.institutionId];
          if (institutionData && institutionData.classrooms) {
            const classroom = institutionData.classrooms.find(c => c.classroomId === support.classroomId);
            if (classroom) {
          ...
          return {
            ...support,
            studentName: `${studentCache[support.studentId].names} ${studentCache[support.studentId].lastNames}`,
            classroomName,
            institutionName: institutionCache[support.institutionId].name,
            diagnosedByName: `Evaluador ${support.diagnosedBy.substring(0, 8)}`,
          };
        } catch (error) {
          console.warn('Error enriching support:', error);
          return {
            ...support,
